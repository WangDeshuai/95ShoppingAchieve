...
-(void)tijiaoBtn{
    ...
    [Engine1 xiuGaiMiMaPhone:_phoneText.text Code:_codeText.text Password:_pwdText.text success:^(NSDictionary *dic) {
...
-(void)codeBtnClick:(UIButton*)sender{
    ...
    [Engine1 getCodePhone:_phoneText.text typeStr:@"3" success:^(NSDictionary *dic) {
        NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
        if ([item1 isEqualToString:@"1"]) {
            [LCProgressHUD hide];
            //实现倒计时
